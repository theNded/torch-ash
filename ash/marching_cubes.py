...
def marching_cubes(
    self, resolution, sdf_fn, weight_fn, bbox_min, bbox_max, dense_grid_dim=8
):
    ...
    sparse_resolution = resolution // dense_grid_dim
    ...
    indices = sparse_indices * dense_grid_dim + dense_indices
    sdfs = sdf_fn(coords)
    weights = weight_fn(coords)
    sparse_dense_grid.embeddings[sparse_indices, dense_indices] = torch.stack(
        (sdfs, weights)
    ).T
